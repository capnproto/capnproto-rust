...
fn macro_usage_three(mut date_list: date_list::Builder) -> Promise<(), capnp::Error> {
    ...
    capnp_build!(date_list, {dates: [{day = 1, month = 2, year_as_text = "1990"},
    ...
    assert_eq!(dates.get(1).get_month(), 3);
    assert_eq!(
        dates.get(2).get_year_as_text().unwrap().to_str(),
        Ok("2023")
    );
...
fn macro_usage_five(mut text_list: text_list::Builder) -> Promise<(), capnp::Error> {
...
async fn capnp_temp_test() -> capnp::Result<()> {
    ...
    macro_usage_five(text_list).await
}
